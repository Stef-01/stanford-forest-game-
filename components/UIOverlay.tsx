...
import { BuildingType, CityStats, Mission, NewsItem, GameMode } from '../types';
...
const CATEGORIES: Record<CategoryKey, { label: string; icon: string; items: BuildingType[] }> = {
  ...
  campus: {
    label: 'Campus',
    icon: '🏛️',
    items: [BuildingType.StudentDorm, BuildingType.LectureHall, BuildingType.ArrillagaHall, BuildingType.HooverTower, BuildingType.EngineeringQuad]
  },
  biz: {
    label: 'Biz',
    icon: '☕',
    items: [BuildingType.CoupaCafe, BuildingType.VapeStore]
  },
  sports: {
    label: 'Sports',
    icon: '🏈',
  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-2 md:p-4 font-sans z-10 text-stone-100">
                ...
                >
                 ...
                 <div className="flex flex-col items-end flex-1">
                    <div className="flex items-center gap-2 w-full justify-end">
                        <span className={`text-sm font-bold font-mono ${stats.wellbeing > 70 ? 'text-blue-400' : stats.wellbeing > 40 ? 'text-yellow-400' : 'text-red-400'}`}>
      ...
      <div className="flex flex-col-reverse md:flex-row md:justify-between md:items-end pointer-events-auto mt-auto gap-2 w-full max-w-full">
        ...
        {/* Condensed Categories Toolbar with Rotation */}
        <div className="flex gap-2 md:gap-4 bg-stone-900/80 p-3 rounded-2xl border border-stone-600/50 backdrop-blur-xl shadow-2xl justify-center w-full md:w-auto flex-wrap items-center">
      ...
      <div className="absolute bottom-1 right-2 md:right-4 text-[8px] md:text-[9px] text-white/30 font-mono text-right pointer-events-auto">
        <a href="https://x.com/ammaar" target="_blank" rel="noreferrer">Stanford Forest Focus</a>
      </div>
    </div>
  );
};
